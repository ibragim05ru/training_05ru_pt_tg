...
def get_stats(start_dt: datetime | None = None, end_dt: datetime | None = None, target_user_id: int | None = None) -> dict:
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()

    conditions = ["is_counted = 1"]
    params: list = []

    if start_dt is not None and end_dt is not None:
        conditions.append("answered_at >= ?")
        conditions.append("answered_at < ?")
    ...
    cur.execute(f"""
        SELECT COUNT(*)
        FROM answers
        WHERE {where_clause} AND is_correct = 1
    """, params)
    ...
    cur.execute(f"""
        SELECT COUNT(*)
        FROM answers
        WHERE {where_clause} AND is_correct = 0
    """, params)
...
def get_user_stats(start_dt: datetime | None, end_dt: datetime | None, user_id: int) -> dict:
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()

    conditions = ["is_counted = 1", "user_id = ?"]
    params: list = [user_id]

    if start_dt is not None and end_dt is not None:
        conditions.extend(["answered_at >= ?", "answered_at < ?"])
        params.extend([start_dt.isoformat(), end_dt.isoformat()])
    ...
    leaderboard_conditions = ["is_counted = 1"]
    leaderboard_params: list = []

    if start_dt is not None and end_dt is not None:
        leaderboard_conditions.extend(["answered_at >= ?", "answered_at < ?"])
        leaderboard_params.extend([start_dt.isoformat(), end_dt.isoformat()])

    leaderboard_where = " AND ".join(leaderboard_conditions)
...
async def cmd_start(message: types.Message) -> None:
    text = (
        "👋 Бот обучения менеджеров\n\n"

        "📊 <b>СТАТИСТИКА</b>\n"
        "/stats_today — за сегодня\n"
        "/stats_yesterday — за вчера\n"
        ...
        "/leaderboard_week — за 7 дней\n"
        "/leaderboard_month — за 30 дней\n"
        "/leaderboard_all — за всё время\n\n"

        "📈 <b>АКТИВНОСТЬ</b>\n"
        "/activity_today — по часам (сегодня)\n"
        "/activity_week — по дням (7 дней)\n"
        "/activity_month — по дням (30 дней)\n\n"

        "👉 Персонально:\n"
        "/activity_week @username\n\n"

        "🧠 <b>ВОПРОСЫ</b>\n"
        "/publish — опубликовать вопрос\n\n"
...
async def cmd_stats_all(message: types.Message) -> None:
    await handle_stats_command(message, "Статистика за всё время", None, None)
